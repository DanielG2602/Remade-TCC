...
<head>
    ...
    <title>Ingredientes — AcervoRCT</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .descricao { color: #6b7280; font-size: 0.85rem; }
    </style>
</head>
<body>
    <header>
    ...
    <main>
        <div class="top">
            <h2>Ingredientes</h2>
            <a class="btn" href="/ingredientes/novo">+ Novo ingrediente</a>
        </div>
        ...
        <table>
            ...
            <tbody>
                <?php if (empty($ingredientes)): ?>
                    <tr><td colspan="4" class="empty">Nenhum ingrediente cadastrado.</td></tr>
                <?php else: ?>
                    <?php foreach ($ingredientes as $i): ?>
                        <tr>
                            <td><?= $i['id'] ?></td>
                            <td><?= htmlspecialchars($i['nome']) ?></td>
                            <td class="descricao"><?= htmlspecialchars($i['descricao'] ?? '—') ?></td>
                            <td>
                                <div class="actions">
                                    <a class="btn btn-secondary" href="/ingredientes/<?= $i['id'] ?>/edit">Editar</a>
                                    <form method="POST" action="/ingredientes/<?= $i['id'] ?>/delete"
                                          onsubmit="return confirm('Excluir este ingrediente?')">
