...
<head>
    ...
    <title><?= isset($ingrediente) ? 'Editar' : 'Novo' ?> Ingrediente — AcervoRCT</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        input, textarea { width: 100%; padding: 0.625rem 0.875rem; border: 1px solid #d1d5db; border-radius: 6px; font-size: 0.9rem; color: #111827; margin-bottom: 1.25rem; }
...
<body>
    ...
    <main>
        ...
        <div class="card">
            ...
            <form method="POST" action="<?= isset($ingrediente) ? '/ingredientes/' . $ingrediente['id'] : '/ingredientes' ?>">
                <label for="nome">Nome *</label>
                <input
                    type="text"
                    id="nome"
                    name="nome"
                    value="<?= htmlspecialchars($ingrediente['nome'] ?? $data['nome'] ?? '') ?>"
                    required
                    autofocus
                >
                ...
                <textarea
                    id="descricao"
                    name="descricao"
                ><?= htmlspecialchars($ingrediente['descricao'] ?? $data['descricao'] ?? '') ?></textarea>

                <div class="actions">
                    <button class="btn btn-primary" type="submit">Salvar</button>
                    <a class="btn btn-secondary" href="/ingredientes">Cancelar</a>
                </div>
            </form>
        </div>
    </main>
